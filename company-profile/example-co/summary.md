...
## Open findings
`probe-cicd-env` ran against `example-co` this run (`provenance.runId` run_01M2GDZ3Q3S5WYCB3MV02QJXZ1; this run's 71 ledger
lines were appended by the `soc-ledger-keeper` at 2026-09-16T12:22:29Z): 29 new finding(s), 0 re-seen (0 superseded, 0
reopened) and 21 observation(s) — 10 `not-satisfied`, 8 `partial`, 1 `satisfied` ([obs_01M2N9V5K8YD6QVKQ6N5SD07ZB]), 1
`not-applicable` ([obs_01M2N9V5K8RQY8DSREZXR01HF4]) and 1 `inconclusive` ([obs_01M2N9V5K8KWKPZ1HBRH913128]) — plus 21
control records, all against repo `mcp-gateway/mongodb-mcp-server` at pinned commit aaa72a040db4c32f3d6488d5e28e2892a68e9ee0
(evidence:
`kpis/data/raw/sessions/opencode/run_01M2GDZ3Q3S5WYCB3MV02QJXZ1.probe-cicd-env.mcp-gateway.mongodb-mcp-server.sarif.export.json`,
sha256 04f11536…). Every new finding is a CI/CD or supply-chain gap on the pipeline that builds, publishes and deploys the
`mongodb-mcp-server` image the qa and prod environments run:

- 14 unpinned-action findings (CICD-PIN-01), one per workflow file — 10 high, 4 medium — including the jobs that hold
  third-party accuracy secrets, pass `DEVTOOLS_BOT_PRIVATE_KEY` to the mutable-branch
  `mongodb-js/devtools-shared/actions/setup-bot-token@main` action (docker.yml, prepare-release.yml) or consume
  `JIRA_API_TOKEN` (jira-issue.yml).
- 5 unpinned build inputs (CICD-PIN-02/03): both Dockerfiles build `FROM node:24-alpine` without a digest, the root
  Dockerfile and the qa/prod deploy image install `mongodb-mcp-server` from the npm registry at mutable `latest` with no
  lockfile or checksum, and the mcp-publisher binary is fetched from `releases/latest` and executed unverified in the
  Production job.
- 2 secrets-in-argv findings (CICD-SEC-02): the Jira bearer token is expanded into a `curl` command-line header in
  publish.yml and prepare-release.yml.
- 8 pipeline-control gaps: no dependency vulnerability scan gate, the container image pushed to Docker Hub with no
  vulnerability scan, the deploy IaC never linted or scanned although the repo record sets `containsIac` true, the
  policy's blocking gitleaks gate not implemented (CI runs git-secrets with AWS patterns only), no usable SBOM for the
  image, two unverified promotion channels into production (npm `@latest` to ECR and the bicep mutable tag) and no
  rollback plan in the release pipeline although `releaseProcess.rollbackPlanRequired` is true.

Four of the unpinned-action rows are recorded `severity: medium`, re-anchored off `sebi-cscrf-2024:PR.DS.S6` (whose CSCRF
catalog applicability exempts mid-size REs) onto `GV.SC.S8` (accuracy-tests.yml, prepare-release.yml, stale.yml) and
`PR.IP.S2` (docker.yml); the Regulatory ref column below quotes each record's re-anchored `regulatoryRefs[0]` verbatim.

Since the previous render of this section (`execute-scr`, 2026-09-15T19:42:44Z), `impl-change-management`
(2026-09-15T23:48:58Z) superseded the 24 findings named by its 10 initiatives' `findingIds`, moving them `open` ->
`triaged`, and `impl-auto-improvement` (2026-09-16T02:18:09Z) superseded the 5 findings its suggestions answer,
preserving status (4 `open`; [fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM] already `triaged`); no finding was closed in between
(0 ledger matches each for `resolved`, `risk-accepted`, `false-positive` and `duplicate`). First seen, severity, target,
primary regulatory ref and SLA due date are unchanged in every supersession (`firstSeenAt` preserved across
supersessions; verified per re-recorded id).

Latest record per finding id in `company-profile/example-co/soc/main.jsonl` with `status` in
`open | triaged | remediating`, as of `provenance.generatedAt` 2026-09-16T12:22:29Z (79 open, 0 past SLA; 24 `triaged`,
55 `open`; 60 high, 19 medium):

### High

| Id | Title | Target | Regulatory ref | First seen | SLA due | SLA status | Status | Initiative |
|---|---|---|---|---|---|---|---|---|
| [fnd_01M2FS4806WHXANC3Q9QXEAXGF] | assurance-expiring: github-cloud SOC 2 Type II report expires 2026-09-30, inside 90-day window | vendor `github` | SEBI sebi-cscrf-2024 GV.SC.S4 (SEBI CSCRF 2024) | 2026-09-14 | 2026-10-14T10:46:39Z | due in 28 d | triaged | n/a |
| [fnd_01M2FS9ECP3E0MTCRQFEJ97930] | contract-expiring: ATLAS-2025-1189 ended 2026-05-31, 106 days before NOW, no renewal evidence | vendor `mongodb-atlas` | SEBI sebi-cscrf-2024 GV.SC.S4 | 2026-09-14 | 2026-10-14T10:46:39Z | due in 28 d | triaged | n/a |
| [fnd_01M2G2AGPX8KWSY1KXGYADK4TK] | find tool returns full unmasked documents from pii/financial collections | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S4 | 2026-09-14 | 2026-09-21T13:14:50Z | due in 6 d | triaged | n/a |
| [fnd_01M2G2AGR581507FMK0EADFKY6] | aggregate tool returns full unmasked pipeline results | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S4 | 2026-09-14 | 2026-09-21T13:14:50Z | due in 6 d | triaged | n/a |
| [fnd_01M2G2AGSC977KASP37WQXYSQA] | export tool writes full unmasked documents to an unencrypted local file | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S4 | 2026-09-14 | 2026-09-21T13:14:50Z | due in 6 d | triaged | n/a |
...
| [fnd_01M2HCWMBDDT80V2CWJ5Z7BH17] | MCP gateway deploys unauthenticated on a public endpoint by default (qa/prod IaC root) | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S17 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBDGA1D4Z54RET9CNC9] | Baseline deployment parameters ship the gateway with no auth and write mode enabled | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S17 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBDCEGN51BKVVXGTV67] | qa/prod deployment template defines no logging, diagnostics or SIEM forwarding | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S8 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | triaged | n/a |
| [fnd_01M2HCWMBDJTN23D3600K6QX9M] | Deployment location not constrained to India for an IN-residency pii/financial workload | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S13 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBDH7QTRQH0XHDBPAJK] | Single replica with autoscaling disabled for the order-routing critical function | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 RC.RP.S1 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBD5YRM4M6TKKT1M974] | Dev-root Dockerfile installs the server and base image unpinned (latest by default) | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S6 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBDBP41GF0TNY5731M8] | AWS deploy-root Dockerfile hardcodes @latest npm install for the qa/prod image | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S6 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HCWMBDEHJMTP3A934TCM7Q] | No owner, data-classification or environment tags on the deployed resources | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 ID.AM.S1 | 2026-09-15 | 2026-09-22T00:05:59Z | due in 6 d | open | n/a |
| [fnd_01M2HRGEX25CQZERDVHPVF1B96] | connect tool accepts a MongoDB connection string with embedded credentials as a plain tool argument | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S6 | 2026-09-15 | 2026-09-22T02:47:35Z | due in 6 d | open | n/a |
| [fnd_01M2HRGF3S0NWNNWDBY1CHD934] | switch-connection tool accepts a connection string with embedded credentials as a plain tool argument | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S6 | 2026-09-15 | 2026-09-22T02:47:35Z | due in 6 d | open | n/a |
| [fnd_01M2HRGFABHFN9JE2TDMJX32RE] | export tool writes query results (pii, financial) to plaintext EJSON files with no encryption or access controls | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S1 | 2026-09-15 | 2026-10-15T02:47:35Z | due in 29 d | triaged | n/a |
| [fnd_01M2K7B0MJ68AR00YTNE2N00V2] | MongoDB MCP confirmation gate fails open: destructive tools execute without human approval | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S3 | 2026-09-15 | 2026-09-22T15:14:31Z | due in 7 d | triaged | n/a |
...
| [fnd_01M2K7B0MJN2XBQRW2XYYD6EBJ] | Setup installs third-party agent skills globally with no integrity check | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S1 | 2026-09-15 | 2026-09-22T15:14:31Z | due in 7 d | triaged | n/a |
| [fnd_01M2K7B0MJQMT1S973WHW7K9GA] | Write tools enabled by default: 11 of 19 write-capable tools have no approval and no allow-list | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S3 | 2026-09-15 | 2026-09-22T15:14:31Z | due in 7 d | triaged | n/a |
| [fnd_01M2K7B0MJJVWFWXF6KZ783GFZ] | connect and switch-connection accept model-controlled connection strings with no host allow-list | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S2 | 2026-09-15 | 2026-09-22T15:14:31Z | due in 7 d | triaged | n/a |
| [fnd_01M2K7B0MJ3239WAE0AKHRMK5E] | Repo ships developer-harness configuration for GitHub Copilot outside harnessesAllowed | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.PO.S1 | 2026-09-15 | 2026-09-22T15:14:31Z | due in 7 d | triaged | n/a |
...
| [fnd_01M2KBJW4AHE221KPTRV1G60YC] | mcp-gateway/mongodb-mcp-server: Human-confirmation gate for destructive MCP tools fails open for clients without elicitation support | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S3 | 2026-09-15 | 2026-09-22T19:42:44Z | due in 7 d | triaged | n/a |
| [fnd_01M2KC7W3QM0V86PVA3SVA01FN] | db-models/onfinance-db-model-master: Cleartext broker credentials modelled in the secrets MongoDB collection | repo `db-models/onfinance-db-model-master` | SEBI sebi-cscrf-2024 PR.DS.S1 | 2026-09-15 | 2026-09-22T19:42:44Z | due in 7 d | triaged | n/a |
...
| [fnd_01M2KM42DZDZSFHCN7YYBM94V2] | Dev environment retains logs only 30 days against the CERT-In 180-day baseline | environment `mcp-gateway/dev` | CERT-In cert-in-directions-2022 Dir-iv (CERT-In Directions 2022) | 2026-09-15 | 2026-10-15T19:42:44Z | due in 30 d | triaged | n/a |
...
| [fnd_01M2KM42DZE09A2PZ557A7SX2F] | .gitignore ignores only the exact name .env, leaving .env* variants and key files trackable in a public repo | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S4 | 2026-09-15 | 2026-09-22T19:42:44Z | due in 7 d | triaged | n/a |
| [fnd_01M2KM42DZ66HX2ZKMY2QQ5JJC] | Repo ships GitHub Copilot and gh-aw harness configuration outside aiCodingPolicy.harnessesAllowed | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.PO.S1 | 2026-09-15 | 2026-09-22T19:42:44Z | due in 7 d | triaged | n/a |
...
| [fnd_01M2N9V5K8XEPT1J7WQPB2SRG3] | Unpinned actions in cleanup-atlas-env.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K84FCJX25WE24VHSAZ] | Unpinned actions in code-health-fork.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K859RHDJWQ56Y562VX] | Unpinned actions in code-health-long-running.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8HZ5TFTTWQ8FFEM1N] | Unpinned actions in code-health.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8K1JR1HKYNZKCMY4H] | Unpinned actions in codeql.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8VF7W5XGZXE695VJK] | Unpinned action in copilot-setup-steps.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 ID.RA.S1 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8F4KQ8VWWYDWH2HT8] | Unpinned actions in jira-issue.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 ID.RA.S1 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8618105KC2S2W9GDV] | Unpinned actions in label-pr.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K87B1VEHSV22Y41T09] | Unpinned actions in mcpb.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S6 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8QY2MHC9CCKT0VM39] | Unpinned actions in the release workflow publish.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.DS.S6 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8T68B7HRCYQD7DY4V] | Root Dockerfile builds from an unpinned node:24-alpine base | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K88WC6QQR9426D07HA] | Prod deploy root Dockerfile builds from an unpinned node:24-alpine base | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8DZYB8XTSDM1SX2ZS] | Image content installed from npm at build time with no lockfile or checksum | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8S8M0Y4GMEKA7NKWC] | Prod deploy image built from npm @latest with no pin | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S3 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K85BQ898RNPMZCS64Q] | mcp-publisher fetched from releases/latest and executed unverified | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K86N0D204M06K09453] | Jira token passed as a curl command-line argument in publish.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S1 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8FXGNKBXCT5J745TR] | Jira token passed as a curl command-line argument in prepare-release.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.AA.S1 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8M2YAPDM472Q5KR6P] | No dependency vulnerability scan gate anywhere in CI | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 ID.RA.S1 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K8N0B1D6MECJBDF8BE] | Container image pushed to Docker Hub with no vulnerability scan | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S12 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K984R66R3X1DPD7715] | Policy blocking gitleaks gate not implemented; CI runs git-secrets with AWS patterns only | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S4 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K9ZW06GFV0368GW0V8] | No usable SBOM for the mongodb-mcp-server image | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S5 | 2026-09-16 | 2026-10-16T12:22:29Z | due in 30 d | open | n/a |
| [fnd_01M2N9V5K98RQJ74DZATYY35YE] | Prod deploy recipe promotes npm @latest with no verification | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S3 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K9XCKDK127TCX81XGR] | Bicep deploys the image by mutable tag with no verification | repo `mcp-gateway/mongodb-mcp-server` | RBI rbi-outsourcing-risk-directions-2025 97(vii) (RBI Outsourcing Directions 2025) | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2N9V5K9KJTBXH98S616WBV3] | No rollback plan in the release pipeline although the policy requires one | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S3 | 2026-09-16 | 2026-09-23T12:22:29Z | due in 7 d | open | n/a |
| [fnd_01M2KM42DZQK8J4EZEB68ZQ103] | Integration-test mongod published unauthenticated on all interfaces of developer laptops | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S1 | 2026-09-15 | 2026-09-22T19:42:44Z | due in 7 d | open | n/a |
...
| Id | Title | Target | Regulatory ref | First seen | SLA due | SLA status | Status | Initiative |
|---|---|---|---|---|---|---|---|---|
| [fnd_01M2FRY4TTS6M3JJD1F1P7J0TQ] | material-without-evidence: aws-mumbai (order-routing, pii/financial) has no documented exit plan | vendor `aws` | SEBI sebi-cscrf-2024 GV.SC.S3 | 2026-09-14 | 2026-12-13T10:46:39Z | due in 88 d | triaged | n/a |
| [fnd_01M2FS481DGTPTR9NJGKAAMF49] | contract-expiring: GH-ENT-2025-07 ended 2026-06-30, 76 days before now, no renewal evidence | vendor `github` | SEBI sebi-cscrf-2024 GV.SC.S3 | 2026-09-14 | 2026-12-13T10:46:39Z | due in 88 d | triaged | n/a |
| [fnd_01M2FS482NWXE7E5MCZY2QMAZ0] | material-without-evidence: github lacks audit rights and a documented exit plan | vendor `github` | SEBI sebi-cscrf-2024 GV.SC.S3 | 2026-09-14 | 2026-12-13T10:46:39Z | due in 88 d | triaged | n/a |
| [fnd_01M2FS9EBVSP2DPWN5K2D91QKW] | material-without-evidence: atlas-mumbai has no audit right and no documented exit plan | vendor `mongodb-atlas` | SEBI sebi-cscrf-2024 GV.SC.S3 | 2026-09-14 | 2026-12-13T10:46:39Z | due in 88 d | triaged | n/a |
| [fnd_01M2G2AGNMHKF9PCB4C85A3JQ3] | MongoDB MCP tool schemas carry no personal-data classification markers | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 ID.AM.S5 | 2026-09-14 | 2026-09-28T13:14:50Z | due in 13 d | triaged | n/a |
| [fnd_01M2G2AGX1GZ9THG6HJ1VPP1EP] | zod tool-argument schemas leave database-bound input unbounded and unvalidated | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S2 | 2026-09-14 | 2026-09-28T13:14:50Z | due in 13 d | triaged | n/a |
...
| [fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM] | db-models/onfinance-db-model-master: no local secrets-scanning hook and no CI, so the policy CI secret scan never reaches this repo | repo `db-models/onfinance-db-model-master` | SEBI sebi-cscrf-2024 PR.IP.S2 | 2026-09-15 | 2026-09-29T19:42:44Z | due in 14 d | triaged | n/a |
| [fnd_01M2N9V5K8WMYXQ0B75TD2MYG6] | Unpinned actions in accuracy-tests.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S8 | 2026-09-16 | 2026-09-30T12:22:29Z | due in 14 d | open | n/a |
| [fnd_01M2N9V5K85030ANM52D7MR410] | Unpinned actions and mutable-branch bot-token action in docker.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S2 | 2026-09-16 | 2026-09-30T12:22:29Z | due in 14 d | open | n/a |
| [fnd_01M2N9V5K899WAGFJN34BBVVEB] | Unpinned actions and bot private key action in prepare-release.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S8 | 2026-09-16 | 2026-09-30T12:22:29Z | due in 14 d | open | n/a |
| [fnd_01M2N9V5K822X97412WMS463HY] | Unpinned actions in stale.yml | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 GV.SC.S8 | 2026-09-16 | 2026-09-30T12:22:29Z | due in 14 d | open | n/a |
| [fnd_01M2N9V5K8ZRG3XZMB2E6E2ZZD] | Deploy IaC never linted or scanned although the repo contains IaC | repo `mcp-gateway/mongodb-mcp-server` | SEBI sebi-cscrf-2024 PR.IP.S6 | 2026-09-16 | 2026-09-30T12:22:29Z | due in 14 d | open | n/a |

<!-- source: latest-state map over soc/main.jsonl kind=finding (1127 lines; 65 records across 50 ids at the previous render + 29 interim supersessions + 29 probe-cicd-env records = 123 finding records across 79 ids), last line per id, filtered to status in open|triaged|remediating; probe-cicd-env batch = lines 1057-1127 recordedAt 2026-09-16T12:22:29Z (1 control line 1057, 21 observations 1058-1078, 29 findings 1079-1107, 20 control records 1108-1127). The 29 new findings verified by grep over main.jsonl: "kind":"finding","id":"fnd_01M2N9V5K = 29 matches (lines 1079-1107), all "status":"open", all "firstSeenAt":"2026-09-16T12:22:29Z", all "target":{"type":"repo","appId":"mcp-gateway","repoId":"mongodb-mcp-server"}; severity via adjacency greps "severity":"high","confidence" = 24 (lines 1080-1085, 1087, 1088, 1089, 1091, 1093-1101, 1103, 1104, 1105, 1106, 1107) and "severity":"medium","confidence" = 5 (lines 1079, 1086, 1090, 1092, 1102); slaDueAt buckets over the 29 ids: 2026-09-23T12:22:29Z = 23, 2026-09-30T12:22:29Z = 5, 2026-10-16T12:22:29Z = 1 (fnd_01M2N9V5K9ZW06GFV0368GW0V8), 2026-12-15T12:22:29Z = 0; primary regulatoryRefs[0] per finding via grep "regulatoryRefs":[{"regulator":"SEBI","instrument":"sebi-cscrf-2024","controlId":"X": GV.SC.S5 6 (lines 1080, 1081, 1093, 1094, 1095, 1104), PR.IP.S4 6 (1082, 1083, 1084, 1088, 1097, 1103), GV.SC.S8 3 (1079, 1090, 1092), PR.IP.S3 3 (1096, 1105, 1107), ID.RA.S1 3 (1085, 1087, 1100), PR.AA.S1 2 (1098, 1099), PR.DS.S6 2 (1089, 1091), PR.IP.S12 1 (1101), PR.IP.S6 1 (1102), PR.IP.S2 1 (1086), plus RBI rbi-outsourcing-risk-directions-2025 97(vii) 1 (1106), sum 29; observation results read record by record (10 not-satisfied, 8 partial, 1 satisfied obs_01M2N9V5K8YD6QVKQ6N5SD07ZB, 1 not-applicable obs_01M2N9V5K8RQY8DSREZXR01HF4, 1 inconclusive obs_01M2N9V5K8KWKPZ1HBRH913128); evidence sarif kpis/data/raw/sessions/opencode/run_01M2GDZ3Q3S5WYCB3MV02QJXZ1.probe-cicd-env.mcp-gateway.mongodb-mcp-server.sarif.export.json sha256 04f11536f8390b4e2cf80e3d1d754b48ff1c955fd50bba2c8ad06f7236670b62. Interim supersessions since the previous render of this section (2026-09-15T19:42:44Z): impl-change-management at 2026-09-15T23:48:58Z superseded the 24 findings named by the 10 initiatives findingIds (grep 24-id alternation .*"status":"triaged" = 24 distinct ids: lines 996-1001, 1008-1016, 1023-1025, 1030, 1031, 1034, 1037, 1039, 1043) and impl-auto-improvement at 2026-09-16T02:18:09Z superseded the 5 suggestion-answered findings (lines 1045-1049; 4 stay open - fnd_01M2HCWMBDDT80V2CWJ5Z7BH17 with "status":"open" at line 1045, likewise 1046, 1047, 1048 - and fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM triaged at 1043 and 1049); grep "status":"(resolved|risk-accepted|false-positive|duplicate)" = 0 matches and "status":"remediating" = 0 matches; slaDueAt preserved in every supersession (verified per re-recorded id: fnd_01M2FRY4TTS6M3JJD1F1P7J0TQ 4 records all 2026-12-13T10:46:39Z at lines 369, 682, 714, 996; fnd_01M2FS4806WHXANC3Q9QXEAXGF 4 records all 2026-10-14T10:46:39Z at 371, 683, 716, 997; fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM 3 records all 2026-09-29T19:42:44Z at 992, 1043, 1049; fnd_01M2HCWMBDDT80V2CWJ5Z7BH17 and fnd_01M2HCWMBDGA1D4Z54RET9CNC9 2 records each all 2026-09-22T00:05:59Z at 762, 763, 1045, 1046; fnd_01M2HRGFABHFN9JE2TDMJX32RE 2 records all 2026-10-15T02:47:35Z at 830, 1011; plus the field-visible re-records at lines 1008-1010, 1025, 1039); the 22 findings not superseded since 2026-09-15T19:42:44Z keep their previous-render values (their latest records are unchanged); SLA status = ceil of slaDueAt minus provenance.generatedAt 2026-09-16T12:22:29Z; counts 60 high + 19 medium = 79 open (24 triaged, 55 open), 0 past SLA (earliest slaDueAt 2026-09-21T13:14:50Z); Initiative column n/a: the finding records carry no initiative field (initiative links live in change_management/master.json findingIds, outside this section's sources) -->

<!-- superseded 2026-09-16T12:22:29Z by the probe-cicd-env open-findings refresh (current source note above); execute-scr's verification note for the 2026-09-15T19:42:44Z render retained verbatim within this comment: source: latest-state map over soc/main.jsonl kind=finding (993 lines, 65 finding records across 50 ids), last line per id, filtered to status in open|triaged|remediating; execute-scr batch = lines 904-993 recordedAt 2026-09-15T19:42:44Z (73 observations 904-917/920-926/928-946/951-969/972-978/985-991, 16 findings 918-919/927/947-950/970-971/979-983/992-993, 1 control 984); severity counts 36 high + 14 medium = 50 open, 0 past SLA. The 16 new rows were verified field-by-field against the ledger: status open (grep "status":"open" over the 16 ids: 20 matches incl. commit_15.diff), firstSeenAt 2026-09-15T19:42:44Z (20 matches), severity via grep "severity":"high" over the 16 ids (8 of 10 non-SDLC rows matched: fnd_01M2KBJW4AHE221KPTRV1G60YC, fnd_01M2KC7W3QM0V86PVA3SVA01FN, fnd_01M2KM42DZQK8J4EZEB68ZQ103, fnd_01M2KM42DZDZSFHCN7YYBM94V2, fnd_01M2KM42DZ98VAGYD0E8TRV3FQ, fnd_01M2KM42DZE09A2PZ557A7SX2F, fnd_01M2KM42DZ66HX2ZKMY2QQ5JJC, fnd_01M2KMNHPKKK5K3C1EYGED2KGG; plus the 2 high SDLC rows read in full) and the SARIF result properties (sha256 1c38ee55…; medium = fnd_01M2KBJW4A8E9GMQHZT9F9WTAG dpdp-rules-2025:3 and fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM sebi-cscrf-2024:PR.IP.S2, plus the 4 medium SDLC rows read in full: fnd_01M2KFQ6B40J52A6C0Q1XFPM10, fnd_01M2KFQ6B43WV47NMR95AXZBT0, fnd_01M2KGVJJ2AXNAXRE08PZ2Z7YP, fnd_01M2KGVJJ2Y4M9DRQY90C7TAMS); slaDueAt verified by grep over the 16 ids: 2026-09-22T19:42:44Z (7 d, sebi-cscrf-2024 PR.MA.S3 patch-sla high) for fnd_01M2KBJW4AHE221KPTRV1G60YC, fnd_01M2KC7W3QM0V86PVA3SVA01FN, fnd_01M2KM42DZQK8J4EZEB68ZQ103, fnd_01M2KM42DZ98VAGYD0E8TRV3FQ, fnd_01M2KM42DZE09A2PZ557A7SX2F, fnd_01M2KM42DZ66HX2ZKMY2QQ5JJC, fnd_01M2KMNHPKKK5K3C1EYGED2KGG; 2026-09-29T19:42:44Z (14 d, patch-sla medium) for fnd_01M2KBJW4A8E9GMQHZT9F9WTAG and fnd_01M2KMNHPJ7Z0Y0YJT6YZR4NZM; 2026-10-15T19:42:44Z (30 d) for fnd_01M2KM42DZDZSFHCN7YYBM94V2 and fnd_01M2KFQ6B4EF1Z113T0NKDY04Q; 2026-12-14T19:42:44Z (90 d) for fnd_01M2KFQ6B4KPYSFR0AR8SV4G4K, fnd_01M2KFQ6B40J52A6C0Q1XFPM10, fnd_01M2KFQ6B43WV47NMR95AXZBT0, fnd_01M2KGVJJ2AXNAXRE08PZ2Z7YP, fnd_01M2KGVJJ2Y4M9DRQY90C7TAMS (the last five read in full at lines 947-950, 970-971); targets verified by grep over the 16 ids (repo mcp-gateway/mongodb-mcp-server x6, repo db-models/onfinance-db-model-master x3) plus company x4 and environment mcp-gateway/dev x1 read in full; regulatoryRefs[0] as tabulated, from the records read in full (SDLC rows) or the SARIF result properties and each record's severity derivation (SCR and dev-env rows). The 34 pre-existing rows are unchanged: no line in 904-993 carries a pre-existing finding id (grep "supersedes":"fnd_ found no match in lines 904-993; this run re-seen 0 findings), so their latest records and the previous section's verified values stand (vendor rows lines 714-721, probe-iac rows 762-769, probe-schemas rows 825-830, first-run schema rows 423-429); their SLA status was recomputed against the new generatedAt: the 5 rows due 2026-09-21T13:14:50Z move 5 d -> 6 d and the 2 rows due 2026-09-28T13:14:50Z move 12 d -> 13 d, all other rows unchanged. Observation result counts 23 not-satisfied + 24 partial + 10 satisfied + 12 not-applicable + 4 inconclusive = 73 (inconclusive: obs_01M2KC7W3PDEXXXW7QC7BHMZE4, obs_01M2KFQ6B4A4R3KCJB4QYH58FR, obs_01M2KGVJJ21P79839X4NKNPAC1, obs_01M2KGVJJ2NB6FNRWWPSJF326D; the results of the 5 dev-env observations not fully visible in place were verified by grep: obs_01M2KM42DTR4YX8F7F0GDPB7QP, obs_01M2KM42DYFQ90GTVVXE6WSMFR, obs_01M2KM42DYNG2Z6PQVZ5CDJVP6 and obs_01M2KMNHPEX22EG5NSKATCAKN6 not-satisfied, obs_01M2KMNHPJB1NRPWQTYPREHRZ8 satisfied). SLA status = ceil((slaDueAt - 2026-09-15T19:42:44Z)/86400000) d; 0 rows past SLA -->
