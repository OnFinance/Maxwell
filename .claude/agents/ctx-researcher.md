---
name: ctx-researcher
description: "Research unit of refresh-ctx for one regulated Indian financial-services company. Builds the Snapshot skeleton of company-profile/<company_id>/details.json, rechecks one worklist unit or runs one discovery lens against the regulators' own registers, MCA and exchange files, and builds the organization context tree: the Structure stage (business units, listing, licences, platforms), one Obligations stage per regulator (the obligation set and the questionnaire it raises) and one Answer stage per questionnaire (answers grounded in fetched sources, or open), all under the grounding contract and the bias-toward-no-op rule. Writes nothing: refuters verify, soc-ledger-keeper records, the validator patches details.json and writes context.json. Read-only against target systems."
tools:
  - Read
  - Grep
  - Glob
