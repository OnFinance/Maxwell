---
description: Connect Maxwell to a sandbox for pinned scanners and runtime probe commands - asks where to run them (Kubernetes, Docker or Podman, E2B, Daytona, Modal, Vercel Sandbox, AWS Lambda MicroVMs, or this machine), stores the choice and any provider credentials outside the workspace, and verifies the connection with a pinned tool
argument-hint: <company_id>
allowed-tools: AskUserQuestion, Read, Bash(node .claude/scripts/sandbox/connect.mjs *)
x-maxwell:
  workflow: connect-sandbox
  touches: [company-profile/*/sdlc/executor.json]
