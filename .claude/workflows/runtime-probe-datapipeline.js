// runtime-probe-datapipeline: read-only runtime probe of a company's data pipelines (outbox, queues, buckets, databases,
// lineage, retention, residency, backup and DR) using the pipeline-prober agent. Never reads a row payload, object or message.
// Scout: company-profile/<c>/sdlc/metastore.json (catalogs, pipelines, lineage) joined with applications/*/env/*.json. Without
// a metastore every target is blocked (METASTORE_MISSING): inconclusive observations plus an evidence request to run
// refresh-metastore. Refute: evidence + regulatory-mapping lenses; high/critical findings, risks and incidents need both;
// assessed observations face the evidence lens in one batch per environment; rules-of-engagement section 7 safety records
...
// Shape: Scout (+ single run timestamp) -> inline rules-of-engagement preconditions (prod gating, readOnly, method,
// credentialKey, changeFreeze, allowedWindows, metastore; every failed check becomes a blocker) -> pipeline per (app, env):
// prober -> refuter lenses -> Dedup barrier -> soc-ledger-keeper (append.mjs with soc-ledger reconciliation, then
// version.mjs) -> report-writer (summary.md).
// Dry run: no command, request or query touches a target; only inconclusive "DRY RUN:" observations are written.
...
export const meta = {
  name: 'runtime-probe-datapipeline',
  description: 'Read-only data-pipeline probe from the metastore: idempotency, checksums, lineage, retention, PII in staging, encryption, access, India residency, DLQs, backup and DR; RBI 2026, SEBI CSCRF.',
  phases: [
    { title: 'Scout', detail: 'pipeline-prober in scout mode joins sdlc/metastore.json catalogs, pipelines and lineage with applications/*/env/*.json to list (app, env) targets and their stores and fixes one run timestamp; the workflow records every failed prod-gating, readOnly, method, credential, freeze, window and metastore check as a blocker' },
    { title: 'Probe', detail: 'pipeline-prober resolves the credential locator and runs its PIP-01..PIP-11 metadata-only checks per store within the rate limit, or plans them in dry-run and blocked modes, returning candidate records keyed by ruleId and evidence requests' },
    { title: 'Refute', detail: 'refuter attacks each candidate through the evidence and regulatory-mapping lenses in parallel (strict majority, unanimity for high/critical findings, risks and incidents) and batch-checks assessed observations through the evidence lens' },
...
const WORKFLOW = 'runtime-probe-datapipeline';
const PROBE_AGENT = 'pipeline-prober';
const CHECK_PREFIX = 'PIP';
...
const DOMAIN = 'data-pipeline';
const ROE_SECTION = 'datapipeline';
const BASE_TAGS = ['runtime-probe', 'data-pipeline'];
const BASE_LENSES = ['evidence', 'regulatory-mapping'];
const SEVERE_LENSES = BASE_LENSES;
const PROBE_METHODS = ['cloud-api', 'kubeconfig'];
const RW_RULE = 'probe-credential-not-read-only'; // as pipeline-prober section 8 names it
const RW_CHECK_ID = 'PIP-RW';
const ALLOWED_TIERS = null; // cross-cutting probe: inherits the tier rule of the environment it touches (prod gating applies)
const LOCATION_EXAMPLES = 's3/<bucket>, sqs/<queue name>, rds/<db identifier>, backup/<vault>, postgres/<db>/<schema>/<table>, kms/<key-alias>';
const RULE_EXAMPLES = '"pipeline-queue-not-idempotent", "pipeline-store-unencrypted", "pipeline-backup-dr-unproven"';
const EXTRA_READS = ', company-profile/' + companyId + '/sdlc/metastore.json (the catalogue of stores, tables, column classifications, pipelines and lineage edges to check reality against), applications/<appId>/images/*.cdx.json (lineage tail) and .claude/skills/reference-architectures/references/investigation-saver-drhp-offline-copy.md section 2 (outbox to ECS run reference pipeline; grade outcomes, not names)';
const regmapHint = () => 'For data-pipeline gaps cite catalog ids, most specific first: for SEBI REs sebi-cscrf-2024 PR.DS.S6 (integrity verification) for PIP-01 to PIP-04 with GV.SC.S5 (SBOM) for the lineage tail; PR.AA.S13 (data retention and disposal) and PR.AA.S8 (log management) for PIP-05; PR.DS.S1 (encryption at rest and in transit, DLP) for PIP-06 and PIP-07; PR.AA.S3 (least privilege) for PIP-08; PR.IP.S13 (SEBI cloud framework, data in India) and PR.DS.S2 for PIP-09; PR.IP.S7, PR.IP.S8 and RC.RP.S1 / RC.RP.S4 (backup, restoration drills, recovery) for PIP-10 and PIP-11. For RBI REs rbi-cyber-tech-directions-2026 data-integrity, cryptography, localisation and BCP/DR clauses (no catalog file: follow the prober definition section 4 rule), rbi-digital-payment-security-2021 and RBI payment-data localisation for payment data, rbi-it-outsourcing-md-2023 16(g) when a provider stores the data. cert-in-directions-2022 Dir-iv for log retention; dpdp-rules-2025 6(1)(a), 6(1)(b), 6(1)(d), 8(1), 8(3), 13(4) or 15 for personal data; pci-dss-4.0.1 only when dataClassification contains cardholder. Then global mappings: nist-csf-2.0 PR.DS-01 / PR.DS-02 / PR.DS-11, nist-800-53-r5 SI-7 / SC-28 / CP-9, cis-controls-8.1. PR.PS and PR.IR are NIST CSF 2.0 ids: under sebi-cscrf-2024 they are mapping errors.';
const defaultRateFor = () => 30;
let scoutMetastore = { present: false, snapshotAt: null };
const targetContext = (t) => 'Metastore: ' + (scoutMetastore.present ? 'snapshot ' + (scoutMetastore.snapshotAt || '?') : 'MISSING') + '. ' + ((t.storeRefs || []).length ? 'Stores in scope (from the metastore): ' + t.storeRefs.join('; ') + '. ' : 'No store is attributed to this environment in the metastore: probe only stores you can tie to it from the env record and say so in notes. ') + ((t.pipelineIds || []).length ? 'Pipelines: ' + t.pipelineIds.join(', ') + '.' : '');
const lensText = (t, c, lens) => 'Lens ' + lens + ': apply the refuter checks for this lens from your agent definition.';
const CHECKS = (t) => 'PIP-01 queue idempotency (ruleId pipeline-queue-not-idempotent): every message carries a per-attempt idempotency key and consumers dedupe (FIFO deduplication or a unique consumer index); without database evidence the check is unknown, never pass; '
  + 'PIP-02 artifact checksums and versioning (pipeline-artifact-not-versioned): versioning Enabled on every artifact bucket, object lock or equivalent write-once policy on output and evidence buckets in prod, artifact rows record sha256 and version id; '
  + 'PIP-03 fingerprint uniqueness (pipeline-fingerprint-not-unique): unique constraints on the record fingerprint and external id per run, duplicate count zero; '
  + 'PIP-04 lineage chain (pipeline-lineage-broken): record -> run -> harness snapshot -> task-definition revision -> image digest -> SBOM attestation intact for recent runs; a tag instead of a digest, a missing attestation or an empty SBOM is a break; '
  + 'PIP-05 retention enforcement (pipeline-retention-not-enforced): lifecycle rules match the stated retention, the enforcement job succeeded within its schedule, logs kept >= 180 days (CERT-In) and personal data not kept past its purpose (DPDP); '
  + 'PIP-06 PII in intermediate storage (pipeline-pii-in-intermediate-store): queues, scratch buckets and staging tables hold references and hashes, not raw pii/spdi/cardholder fields, or are CMK-encrypted, short-lived and classified; Macie types and counts only; '
  + 'PIP-07 encryption at rest and in transit (pipeline-store-unencrypted): every store encrypted with a rotating CMK held in an allowed region, TLS enforced (rds.force_ssl=1, aws:SecureTransport deny); '
  + 'PIP-08 access to buckets and databases (pipeline-store-overexposed): no public access, no Principal "*" without a VPC-endpoint or org condition, databases not publicly accessible, application roles cannot delete from append-only tables (grants by evidence request); '
  + 'PIP-09 residency (pipeline-data-outside-residency): every store, replica, backup and queue inside residency ' + ((t.residency || []).join('/') || '?') + ', payment data only in India regions; '
  + 'PIP-10 dead-letter queues (pipeline-dlq-missing-or-undrained): a DLQ with bounded maxReceiveCount per job queue, oldest DLQ message younger than the redrive SLA (reference 24 h), a depth alarm; '
  + 'PIP-11 backup and DR evidence (pipeline-backup-dr-unproven): automated backups at or above policy retention, a completed backup within 24 h, a successful restore test within the drill cadence, backups encrypted, vault-locked and inside residency. '
  + 'Use no other check ids; checks that need SQL (PIP-01 counts, PIP-03 indexes, PIP-04 null snapshots, PIP-05 overdue rows, PIP-08 grants) become inconclusive with an evidenceRequest for the exact metadata-only query output run by the data-platform team, with sha256.';
const EXTRA_PROHIBITIONS = () => 'never read an object, a message or a row payload (no s3api get-object, head-object or select-object-content, no s3 cp/sync/ls of contents, no sqs receive-message, purge-queue or change-message-visibility, no kms decrypt, secretsmanager get-secret-value or ssm get-parameter --with-decryption); never trigger a backup, restore, failover or lifecycle action; psql, mysql, any database client and cosign are allowed in narrow forms by rules-of-engagement section 2 but forbidden by pipeline-prober\'s own allow-list (stricter), so database-internal facts become evidence requests.';
const EXTRA_PROMPT = () => '';
const SCOUT_EXTRA_PROPS = { metastorePresent: { type: 'boolean' }, metastoreSnapshotAt: { type: 'string' } };
const SCOUT_TARGET_PROPS = { storeRefs: { type: 'array', items: { type: 'string' } }, pipelineIds: { type: 'array', items: { type: 'string' } } };
const scoutScopeLines = [
  'Read company-profile/' + companyId + '/sdlc/metastore.json if it exists (return metastorePresent and metastoreSnapshotAt). An application is in scope when the metastore attributes a catalog to it (catalogs[].appId), names it as a pipeline sourceRepo.appId, or when any of its env records has dataClassification containing pii, spdi, cardholder, financial or regulatory. Without a metastore use the env-record rule alone. Every other app gets a skipped entry {appId, reason: "no metastore catalog, pipeline or regulated data classification"}.',
  'For each in-scope app read every applications/<app_id>/env/<env_id>.json and add to each target storeRefs (metastore tables and lineage datasets whose location, endpointRef or namespace belongs to that environment, as "<catalogId>:<fullyQualifiedName> <location> [classifications] retentionDays=<n>"; queues, buckets and databases named in lineage namespaces too) and pipelineIds (metastore pipelines whose sourceRepo or lineage jobs touch those stores). Report store names, locations and classifications only; never sample data.',
];
const afterScout = (s) => {
  scoutMetastore = { present: s.metastorePresent === true, snapshotAt: s.metastoreSnapshotAt || null };
  if (!scoutMetastore.present) log('sdlc/metastore.json is missing for ' + companyId + ': every target is blocked (METASTORE_MISSING); run /refresh-metastore ' + companyId + ' before a live data-pipeline probe');
};
const extraBlockers = () => (scoutMetastore.present ? [] : ['METASTORE_MISSING: company-profile/' + companyId + '/sdlc/metastore.json does not exist, so the stores, queues and tables of this pipeline are unknown; run refresh-metastore first']);
const afterTargets = () => {};
const afterProbe = () => {};
...
const commandRule = (t) => 'Commands: exactly the families listed in section 2 of ' + AGENT_DEF + ', and only where rules-of-engagement section 2 (' + ROE + ') also allows them for method ' + t.method + ' and tier ' + t.tier + '; where the two lists differ the stricter wins, and anything in neither is forbidden. Every command runs through node .claude/scripts/sandbox/exec.mjs --company <company_id> --app <app_id> --env <env_id> -- <command> [--pipe jq <redaction filter>] (sandbox-executors skill), which applies the 60-second timeout and the 1 MiB cap; there is no shell, so no `&&`, `;` or redirection. A fact only a forbidden command could show is inconclusive with an evidence request, never a pass and never a fail. Workflow-level prohibitions on top of those lists: ' + EXTRA_PROHIBITIONS(t);
