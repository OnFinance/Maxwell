// runtime-probe-qa-env: read-only runtime probe of running container workloads (Kubernetes / Docker hosts) for one company.
// Tier-filtered: only environments whose tier is qa, uat or staging; every other tier is logged as skipped with the workflow that owns it.
// args: { companyId (required), appIds?: string[], envIds?: string[] (bare "uat-mumbai" or qualified "trading-api/uat-mumbai"), dryRun?: boolean,
...
export const meta = {
  name: 'runtime-probe-qa-env',
  description: 'Read-only runtime probe of running containers in qa, uat and staging environments only; CNT-01..CNT-11 checks incl. pod security admission, mapped to SEBI CSCRF and RBI 2026.',
  phases: [
...
const WORKFLOW = 'runtime-probe-qa-env';
const PROBE_AGENT = 'container-prober';
const TIER_GROUPS = { devtest: ['dev', 'test', 'devtest'], qa: ['qa', 'uat', 'staging'], prod: ['prod', 'dr'] };
const TIER_FILTER = ['qa']; // null = every tier group; otherwise the only group this workflow may touch
