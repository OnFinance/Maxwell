...
const PROBE_CHECKS = [
  ...
  'Render first (helm template <chart> -f values.yaml for each values file; kustomize build <overlay>) and scan the rendered output. Prefer the scanners your agent definition permits when installed: kube-linter lint --format sarif, checkov -d <dir> --framework helm,kubernetes -o sarif, trivy config --format sarif, kube-score score and kubeconform (convert their JSON output into SARIF results). If none is installed, review rendered manifests manually (yq/jq) and write the SARIF yourself (tool name "maxwell-chart-auditor").',
