...
const offlineScannerNote = 'Scanners run only offline with local rules: semgrep --metrics=off --config <the repo .semgrep/ directory or a local rules directory> (never --config auto, p/ or r/ registry packs), trivy fs --offline-scan --skip-db-update, grype dir:<path> only with a cached database, bandit, gosec, brakeman, gitleaks detect --no-git --redact. Scanners run only through node .claude/scripts/toolchain/scan.mjs at their pinned versions (scanner-toolchain skill; brakeman is not pinned). A tool that is missing or needs the network goes to skipped with the reason.';
