// probe-iac: static probe of infrastructure-as-code (Terraform/OpenTofu, CloudFormation, Pulumi, Ansible, CDK) in every application repo checkout for one company.
...
// Shape: Scout (repos with checkouts, applicable instruments, run timestamp) -> per-repo pipeline: iac-auditor probe ->
...
export const meta = {
  name: 'probe-iac',
  description: 'Static probe of Terraform/OpenTofu, CloudFormation, Pulumi, Ansible and CDK in every app repo checkout; misconfigs mapped to SEBI CSCRF, RBI Directions 2026 and CIS 8.1.',
  phases: [
    ...
    { title: 'Probe', detail: 'iac-auditor reviews each checkout, exports SARIF, returns candidate observations and findings' },
...
const WORKFLOW = 'probe-iac';
const PROBE_AGENT = 'iac-auditor';
const BASE_TAGS = ["iac","static-probe"];
const TARGET_NOUN = 'infrastructure-as-code';
const SUMMARY_NOUN = 'IaC misconfigurations';
const MAPPING_RULE = '';
const requiredMappingGap = () => '';
const SCOUT_EXTRA = '';
const TARGET_RULE = 'For every applications/<app_id>/repos/<repo_id>.json decide whether it is a target. A repo is a target when its record has "containsIac": true, or when any applications/<app_id>/env/<env_id>.json lists an "iac" root whose repoId is this repo with tool terraform, opentofu, cloudformation, pulumi, ansible or cdk (if args.envIds is given, only those env files count). For each target also list the IaC roots (directories containing *.tf, *.tfvars, template.yaml/*.cfn.yaml, Pulumi.yaml, playbooks/*.yml, cdk.json) relative to the checkout.';
const WRITE_EXTRA = '';
const PROBE_CHECKS = [
  'Checklist:\n- Storage and databases: public S3/GCS/Blob ACLs or policies, block-public-access disabled, missing server-side encryption or customer-managed KMS keys, RDS/Aurora/Cloud SQL without storage encryption, deletion protection or automated backups, snapshots shared publicly.\n- Network: security groups / NSGs / firewall rules open to 0.0.0.0/0 or ::/0 on SSH, RDP, database or management ports; VPC flow logs off; load balancers without TLS 1.2+ policies; public IPs on regulated workloads.\n- Identity: IAM policies with "Action": "*" or "Resource": "*", inline user policies, long-lived access keys, roles trusting "*" principals, missing MFA conditions on sensitive actions, Kubernetes cluster-admin bindings in IaC.\n- Logging and monitoring: CloudTrail/audit logs disabled or not multi-region, log buckets without object lock, retention under 180 days (cert-in-directions-2022:Dir-iv requires ICT logs retained for 180 days within India), GuardDuty/Security Hub/Defender disabled.\n- Data localisation and transfers: payment system data stored or replicated outside India, i.e. regions other than ap-south-1/ap-south-2 (or the non-IN regions of Azure/GCP) for payment workloads (RBI Storage of Payment System Data circular, 6 April 2018; cite it through rbi-cyber-tech-directions-2026 or the catalog clause that carries it, e.g. rbi-it-outsourcing-md-2023:16(g), and sebi-cscrf-2024:PR.DS.S2 / PR.IP.S13 for SEBI REs); personal-data transfers to countries restricted under DPDP Rules 2025 Rule 15 (dpdp-rules-2025:15), and Rule 13(4) only when the company is a notified Significant Data Fiduciary; cross-region replication or backups to foreign regions for such data.\n- Secrets and state: hard-coded credentials in *.tfvars, variables with default secrets, remote state without encryption or locking, state files committed, provider credentials in plain text.\n- Hygiene: unpinned provider/module versions, modules pulled from unverified public registries without version constraints, missing mandatory tags (owner, data-classification, environment), lifecycle prevent_destroy absent on regulated data stores.',
  'Prefer real scanners when your agent definition permits them and they are on PATH: checkov (checkov -d <root> -o sarif), trivy config (trivy config --format sarif <root>), tfsec (--format sarif), tflint (--format sarif), cfn-lint (--format sarif), ansible-lint (-f sarif). When none is installed, do a manual pattern review with Grep and Read (jq/yq for JSON/YAML) and write the SARIF yourself (tool name "maxwell-iac-auditor").',
  'Cite the most specific Indian instrument first: sebi-cscrf-2024 for SEBI REs (PR.DS.S1 encryption, PR.DS.S2 data classification and localisation, PR.AA.S1-S3 identity, segmentation and least privilege, PR.IP.S1 baseline hardening, PR.IP.S7/S8 and RC.RP.S4 backups, PR.AA.S9 and DE.CM.S2 logging and monitoring, PR.IP.S13 cloud adoption), rbi-cyber-tech-directions-2026 and rbi-it-outsourcing-md-2023 (App-I.6 cloud controls) for banks/NBFCs, irdai-info-cyber-security-2023 for insurers, cert-in-directions-2022 (Dir-i clock sync, Dir-iv log retention) as cross-cutting, dpdp-rules-2025 (6(1)(a)-(e) safeguards, 15 transfers) for personal data; then cis-controls-8.1 and nist-800-53-r5 mappings.',
];
