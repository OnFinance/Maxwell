// runtime-probe-sandboxes: read-only runtime probe of agent and code-execution sandbox isolation for one company, using the
// sandbox-prober agent. Rules of engagement section 1 limits this workflow to environments whose tier is "sandbox".
// Scout: every applications/*/env/*.json (non-sandbox tiers are logged as skipped). Refute: evidence + regulatory-mapping
// lenses; findings survive a strict majority, high/critical findings, risks and incidents need both; assessed observations
// face the evidence lens in one batch per environment; rules-of-engagement section 7 safety records are written directly.
// args: { companyId (required), appIds?: string[], envIds?: string[], dryRun?: boolean, now?: RFC 3339 UTC 'Z' string (the
//         run timestamp for windows, freezes and records; when absent the scout reads `date -u` exactly once),
//         sessionId?: string, runId?: string }
// Shape: Scout (+ single run timestamp) -> inline rules-of-engagement preconditions (tier, readOnly, method, credentialKey,
// changeFreeze, allowedWindows, rateLimitPerMinute; every failed check becomes a blocker) -> pipeline per (app, env):
...
export const meta = {
  name: 'runtime-probe-sandboxes',
  description: 'Read-only probe of sandbox-tier isolation: UID, seccomp/LSM, runtime backend, egress and prod separation, tmpfs, leases and TTLs, guards, scan gate, limits and prod data; SEBI CSCRF, RBI 2026.',
  phases: [
    { title: 'Scout', detail: 'sandbox-prober in scout mode lists every (app, env) from applications/*/env/*.json with the sandbox families it runs and fixes one run timestamp; the workflow keeps tier sandbox only and records every failed readOnly, method, credential, freeze and window check as a blocker' },
    { title: 'Probe', detail: 'sandbox-prober resolves the credential locator and runs its SBX-01..SBX-11 read-only isolation checks within the rate limit, or plans them in dry-run and blocked modes, returning candidate records keyed by ruleId and evidence requests' },
...
const WORKFLOW = 'runtime-probe-sandboxes';
const PROBE_AGENT = 'sandbox-prober';
const CHECK_PREFIX = 'SBX';
...
const DOMAIN = 'sandbox-isolation';
const ROE_SECTION = 'sandboxes';
const BASE_TAGS = ['runtime-probe', 'sandbox'];
const BASE_LENSES = ['evidence', 'regulatory-mapping'];
const SEVERE_LENSES = BASE_LENSES;
const PROBE_METHODS = ['kubeconfig', 'cloud-api', 'docker-socket', 'ssh'];
const RW_RULE = 'probe-credential-not-read-only'; // as sandbox-prober section 8 names it
const RW_CHECK_ID = 'SBX-RW';
const ALLOWED_TIERS = ['sandbox']; // rules of engagement section 1: runtime-probe-sandboxes => sandbox tier only
const LOCATION_EXAMPLES = 'ecs/task-definition/<family>, <namespace>/job/<name>, asg/<group>, netpol/<namespace>/<policy>, sg/<security-group-id>, image/<imageId>';
const RULE_EXAMPLES = '"sandbox-runs-as-root", "sandbox-network-reaches-prod", "sandbox-holds-production-data"';
const EXTRA_READS = ', .claude/skills/reference-architectures/references/investigation-saver-drhp-offline-copy.md sections 3 and 7 (sandboxing reference design; grade outcomes, not that design), and every prod or dr environment record of this app (the CIDRs, accounts and secrets backends the sandbox must not reach)';
const regmapHint = () => 'For sandbox gaps cite catalog ids, most specific first: for SEBI REs sebi-cscrf-2024 PR.IP.S1 (baseline configuration, hardening, least functionality) for SBX-01, 02, 03, 05, 07, 08 and 10; PR.AA.S2 (network segregation and segmentation) for SBX-04; PR.DS.S5 (non-production separated from production) for SBX-06 and SBX-11, with PR.AA.S3 (least privilege) when the sandbox identity reaches prod secrets; PR.IP.S4 (scan for malicious code) with DE.CM.S2 for SBX-09. For RBI REs rbi-cyber-tech-directions-2026 secure-configuration, network-security, environment-segregation and anti-malware clauses (no catalog file: follow the prober definition section 4 rule). dpdp-rules-2025 6(1)(a) and 6(1)(b) when unmasked personal data sits in a sandbox; cert-in-directions-2022 Dir-iv for sandbox execution and guard self-test logs. Then global mappings: nist-csf-2.0 PR.PS-01 / PR.IR-01, cis-controls-8.1, nist-800-53-r5 SC-7 / SC-39 / CM-7, iso-27001-2022 A.8.31 / A.8.33, owasp-agentic-top10-2026. PR.PS and PR.IR are NIST CSF 2.0 ids: under sebi-cscrf-2024 they are mapping errors.';
const defaultRateFor = () => 30;
const targetContext = (t) => ((t.sandboxFamilies || []).length ? 'Sandbox families in scope: ' + t.sandboxFamilies.join(', ') + '. ' : 'No sandbox family was named in the workspace: discover them read-only from task definitions, namespaces or host groups and list what you found in notes. ') + ((t.prodEnvRefs || []).length ? 'Production environments the sandbox must be isolated from: ' + t.prodEnvRefs.join('; ') + '.' : 'No prod or dr environment record exists for this app: SBX-04 and SBX-11 compare against the company\'s other prod accounts only if the workspace names them, otherwise record them inconclusive with an evidence request.');
const lensText = (t, c, lens) => 'Lens ' + lens + ': apply the refuter checks for this lens from your agent definition.';
const CHECKS = (t) => 'SBX-01 user and UID (ruleId sandbox-runs-as-root): numeric non-zero UID and GID, runAsNonRoot true, no docker or root supplemental group; '
  + 'SBX-02 seccomp and LSM (sandbox-seccomp-or-lsm-unconfined): RuntimeDefault or Localhost seccomp and an enforcing AppArmor or SELinux profile on every sandbox container; live /proc proof is an evidence request; '
  + 'SBX-03 isolation backend (sandbox-weak-isolation-backend): gVisor, Kata, Firecracker or Fargate micro-VM, or plain runc with SBX-01, 02, 05, 06, 07 and 08 all passing on single-use hosts; a mismatch with an sdlc/policy.json mandate fails; '
  + 'SBX-04 network isolation from prod (sandbox-network-reaches-prod): default-deny egress, egress only to DNS, the governed gateway and named internal ports, no 0.0.0.0/0 egress, no route or VPC peering to the prod CIDRs above, IMDSv2 with hop limit 1; '
  + 'SBX-05 writable paths on tmpfs (sandbox-writable-persistent-path): read-only root FS, every writable path tmpfs or a size-limited emptyDir, no hostPath except read-only CA bundles; '
  + 'SBX-06 single-use hosts and expiry (sandbox-host-reused-or-unexpired): a fresh host or micro-VM per run or recycling after every lease, no host older than the recycle window, every sandbox carries a TTL label, lease or ttlSecondsAfterFinished and none has outlived it (expired sandboxes are gone); lease-table figures by evidence request; '
  + 'SBX-07 init and process guards (sandbox-process-guard-missing): PID-1 init, guard present with the expected hash and its --self-test passed at the last start, no debugger or nsenter in the image SBOM; '
  + 'SBX-08 dumpable and ptrace (sandbox-ptrace-or-dumpable): PR_SET_DUMPABLE 0 (or suid_dumpable 0 with core limit 0), ptrace_scope >= 1 by evidence request, no SYS_PTRACE; '
  + 'SBX-09 workspace scan gate (sandbox-no-scan-gate): untrusted inputs scanned (ClamAV-style workspace-scan init or dependent container with SUCCESS condition) before the agent starts, signatures refreshed within 7 days; '
  + 'SBX-10 resource ceilings (sandbox-no-resource-ceiling): CPU, memory, ephemeral-storage and pids limits on every sandbox container, LimitRange or ResourceQuota in the namespace; '
  + 'SBX-11 no production data or secrets (sandbox-holds-production-data): env dataClassification ' + ((t.dataClassification || []).join('/') || '?') + ' holds no pii, spdi, cardholder or financial data unless masked, a synthetic seed job is the only data source (job names and images, never data), and the sandbox identity cannot reach the prod secrets backends, paths or accounts above (secrets-backend references and credential paths compared by name only). '
  + 'Rules-of-engagement section 9 "sandboxes" TTL and expiry items belong to SBX-06 and prod-secrets-backend items to SBX-11; use no other check ids.';
const EXTRA_PROHIBITIONS = () => 'never exec, attach, cp or debug into a sandbox; never send traffic from inside a sandbox to test egress (prove egress from policy, security-group and route configuration); never ls, stat or read workspace, home or /tmp paths; never restart, drain, cordon or terminate a host; never inspect the process table of a host or task running a customer job (sample at most three idle hosts per family, leave at once with HOST_NOT_IDLE if one starts a run, and note liveRunSkipped and the sampling cap in notes); /proc, sysctl, aa-status, sestatus and runsc reads are outside sandbox-prober\'s commands and become evidence requests.';
const EXTRA_PROMPT = () => '';
const SCOUT_EXTRA_PROPS = {};
const SCOUT_TARGET_PROPS = { sandboxFamilies: { type: 'array', items: { type: 'string' } }, prodEnvRefs: { type: 'array', items: { type: 'string' } } };
const scoutScopeLines = [
  'Read every applications/<app_id>/env/<env_id>.json of every considered app and return all of them as targets, whatever their tier (the workflow keeps tier "sandbox" only and logs the rest). For each target add sandboxFamilies (task-definition families, Kubernetes namespaces, runtime classes or host groups that run agent or untrusted code, taken from the env hosting block, applications/<app_id>/images/*.json and applications/<app_id>/README.md; empty when none is named) and prodEnvRefs: for the same app, every prod or dr environment as "<envId>: <provider>/<region>/<accountRef>/<cluster>/<namespace>, secretsBackend <backend>". An app with no environment files gets a skipped entry {appId, reason: "no env records"}.',
];
const afterScout = () => {};
const extraBlockers = () => [];
const afterTargets = () => {};
