...
const PROBE_CHECKS = [
  ...
  'Run scanners only through node .claude/scripts/toolchain/scan.mjs at their pinned versions (scanner-toolchain skill): zizmor --format sarif for GitHub Actions, actionlint, hadolint for Dockerfiles, checkov -d {src} -o sarif, trivy config --format sarif; semgrep only with rules from the checkout (registry packs such as p/github-actions are refused). For SBOM checks read the CycloneDX JSON with jq (jq \'.components | length\' <file>; a parse error, {} or 0 means empty). Where scan.mjs cannot run a tool (exit 3) write the SARIF yourself (tool name "maxwell-cicd-auditor").',
