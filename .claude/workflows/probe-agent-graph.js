// probe-agent-graph: static probe of LLM agent graph in every application repo checkout for one company.
...
// Shape: Scout (repos with checkouts, applicable instruments, run timestamp) -> per-repo pipeline: agent-graph-auditor probe ->
...
export const meta = {
  name: 'probe-agent-graph',
  description: 'Static probe of LLM agent code per app repo: agents, tools, MCP servers, prompts, memory and egress mapped to OWASP Agentic Top 10 2026 (ASI01-ASI10), OWASP LLM Top 10 2025 and CSA MCP guidance.',
  phases: [
    ...
    { title: 'Probe', detail: 'agent-graph-auditor reviews each checkout, exports SARIF, returns candidate observations and findings' },
...
const WORKFLOW = 'probe-agent-graph';
const PROBE_AGENT = 'agent-graph-auditor';
const BASE_TAGS = ["agentic","static-probe"];
const TARGET_NOUN = 'LLM agent graph';
const SUMMARY_NOUN = 'agent-graph (OWASP Agentic Top 10 2026) gaps';
const MAPPING_RULE = 'OWASP Agentic rule: the finding must cite {regulator: OWASP, instrument: owasp-agentic-top10-2026, controlId: ASI01..ASI10} whose id matches its owasp-agentic-top10-2026:asiNN tag and the weakness described (e.g. prompt injection via tool output is ASI01, over-broad tool side effects ASI02, unpinned MCP servers ASI04). Correct a wrong ASI id through correctedRegulatoryRefs; refute when no ASI id fits.';
const requiredMappingGap = (f) => {
  const refs = (f.regulatoryRefs || []).filter((r) => r && r.instrument === 'owasp-agentic-top10-2026' && /^ASI(0[1-9]|10)$/.test(r.controlId || ''));
  if (!refs.length) return 'no owasp-agentic-top10-2026 ASI01..ASI10 regulatoryRef';
  const tags = (f.tags || []).filter((x) => /^owasp-agentic-top10-2026:asi(0[1-9]|10)$/.test(x));
  if (tags.length !== 1) return 'needs exactly one owasp-agentic-top10-2026:asiNN tag, found ' + tags.length;
  if (!refs.some((r) => 'owasp-agentic-top10-2026:' + r.controlId.toLowerCase() === tags[0])) return 'ASI tag ' + tags[0] + ' does not match the cited ASI controlId';
  return '';
};
const SCOUT_EXTRA = '';
const TARGET_RULE = 'For every applications/<app_id>/repos/<repo_id>.json decide whether it is a target. A repo is a target when its record has "containsAgentCode": true, or when the checkout contains agent-framework markers: prompt files (prompts/, *.prompt.md, system prompts), tool or function definitions for an LLM, MCP server configs (.mcp.json, mcp.json, opencode.json with mcp), agent framework imports (langchain, langgraph, llamaindex, crewai, autogen, openai-agents, anthropic tool_use, vercel ai sdk, semantic-kernel), or harness catalogues. For each target list the agent roots (directories holding agent definitions, tools, prompts, MCP configs) relative to the checkout.';
const WRITE_EXTRA = 'Reject any finding whose tags lack an "owasp-agentic-top10-2026:asiNN" tag; report it in skipped instead of writing it.';
const PROBE_CHECKS = [
  'Checklist:\nMap the agent graph first: every agent (name, model, system prompt path), every tool (name, side effects, allow/deny), every MCP server (transport, auth, pinned version), memory stores, inter-agent channels, human approval steps and kill switches. Then check, tagging each finding with the OWASP Agentic Top 10 2026 id it maps to (tags like "owasp-agentic-top10-2026:asi01"):\n- ASI01 Agent Goal Hijack: untrusted content (web pages, documents, tool output, customer messages) concatenated into prompts without delimiting or content filtering; no instruction/data separation.\n- ASI02 Tool Misuse: tools with broad side effects (shell, file write, HTTP, payment/transfer, KYC update) exposed without allow-lists, parameter validation or scoped credentials; forbidden-tool lists not enforced for browser or code variants.\n- ASI03 Identity and Privilege Abuse: agents running with a shared service identity, static API keys in code or config, no per-tool least privilege, credentials readable by the model context, missing egress classes (offline / loopback-only / governed).\n- ASI04 Agentic Supply Chain: MCP servers or agent packages pulled unpinned (npx -y latest, git main), no integrity check on prompt or tool packages, third-party skills loaded at runtime.\n- ASI05 Unexpected Code Execution: eval/exec of model output, code-interpreter tools without sandbox, templating that renders model text into shell or SQL.\n- ASI06 Memory and Context Poisoning: long-term memory written from untrusted output without validation, shared vector stores across tenants, RAG sources without provenance.\n- ASI07 Insecure Inter-Agent Communication: agent-to-agent messages unauthenticated or unsigned, orchestration channels reachable from the network, no schema validation on handoffs.\n- ASI08 Cascading Failures: no max turns, token budget or recursion limit; retries without backoff; one agent\'s failure fanning out without circuit breakers.\n- ASI09 Human-Agent Trust Exploitation: governor-style agents applying changes instead of proposing them, missing human apply step for regulated actions (customer communication, fund movement, KYC decisions), UI that hides tool calls from operators.\n- ASI10 Rogue Agents: no brakes (*_paused flags, kill switch), no observability of tool calls, prompt/tool content logging off with no audit alternative, agents that can spawn agents without limits.\nAlso note gaps against owasp-llm-top10-2025 (LLM01 prompt injection, LLM02 sensitive information disclosure, LLM06 excessive agency) and csa-mcp-security-2025 for MCP servers. Read sections 3 (agent harness and platform) and 7 (probe checklist) of .claude/skills/reference-architectures/references/investigation-saver-drhp-offline-copy.md for the reference harness controls.',
  'Use Grep and Read over the agent roots (jq/yq for MCP and harness configs); run semgrep --config p/python or p/typescript --sarif when installed. Write the SARIF yourself when no tool exists (tool name "maxwell-agent-graph-auditor"). Never invoke the agents, MCP servers or models under review.',
  'Every finding MUST carry a regulatoryRef with instrument owasp-agentic-top10-2026 and controlId ASI01..ASI10 (regulator OWASP). List the most specific applicable Indian instrument first (sebi-cscrf-2024 PR.AA.S3 least privilege, PR.AA.S17 API security, PR.IP.S15 application security audit, ID.AM.S3 no shadow IT, EV.ST.S1 attack-surface reduction for SEBI REs; rbi-cyber-tech-directions-2026 or rbi-it-outsourcing-md-2023 8 third-party dependency inventory for banks/NBFCs; irdai-info-cyber-security-2023 for insurers; dpdp-rules-2025 13(3) for algorithmic due diligence where personal data is processed), then owasp-agentic-top10-2026, csa-mcp-security-2025 where MCP is involved, and owasp-llm-top10-2025 or nist-ai-600-1 (eu-ai-act-2024-1689 only when the company operates in the EU).',
  'OWASP Agentic Top 10 2026 rule: each finding\'s tags array must include exactly one "owasp-agentic-top10-2026:asiNN" tag (NN = 01..10) and its regulatoryRefs must include {regulator: "OWASP", instrument: "owasp-agentic-top10-2026", controlId: "ASINN"}. A finding with no ASI mapping is not an agent-graph finding: drop it or reclassify it under owasp-llm-top10-2025.',
];
