...
const PROBE_CHECKS = [
  ...
  'Use Grep and Read over the agent roots (jq/yq for MCP and harness configs); run semgrep only through node .claude/scripts/toolchain/scan.mjs at its pinned version (scanner-toolchain skill) with rules from the checkout ({src}/...); registry packs such as p/python are not pinned and are refused. Write the SARIF yourself when scan.mjs cannot run a tool (exit 3) (tool name "maxwell-agent-graph-auditor"). Never invoke the agents, MCP servers or models under review.',
