// runtime-probe-appcontainers: read-only runtime probe of running container workloads (Kubernetes / Docker hosts) for one company.
// Cross-cutting: runs the tier logic inline for every tier group requested (devtest, qa, prod); prod/dr environments are still probed only when args.envIds names them.
// args: { companyId (required), appIds?: string[], envIds?: string[] (prod/dr environments are probed only when named here, bare "prod-mumbai" or qualified "trading-api/prod-mumbai"), dryRun?: boolean,
...
export const meta = {
  name: 'runtime-probe-appcontainers',
  description: 'Read-only runtime probe of running container workloads across devtest, qa and prod tiers: image digests vs registered images, hardening, secrets, logging, patch level and SBOMs.',
  phases: [
...
const WORKFLOW = 'runtime-probe-appcontainers';
const PROBE_AGENT = 'container-prober';
const TIER_GROUPS = { devtest: ['dev', 'test', 'devtest'], qa: ['qa', 'uat', 'staging'], prod: ['prod', 'dr'] };
const TIER_FILTER = null; // null = every tier group; otherwise the only group this workflow may touch
