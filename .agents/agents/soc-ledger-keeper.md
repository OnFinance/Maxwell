...
description: >-
  The only agent that appends to company-profile/<company_id>/soc/main.jsonl and
  writes soc/versions/commit_<n>.diff. Every workflow hands it proposed control,
  observation, finding, risk and incident records; it mints ids, recomputes
  finding fingerprints, checks supersedes chains, provenance, regulatoryRefs and
  SLA fields, then appends via node .claude/scripts/soc/append.mjs and versions
