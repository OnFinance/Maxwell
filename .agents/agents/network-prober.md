...
description: >-
  Read-only runtime probe of one application environment's network perimeter at
  a regulated Indian financial-services company. Spawned by
  runtime-probe-network-perimeter. Enumerates exposure from configuration
  (security groups, NACLs, load balancers, WAF, DNS, VPC routing, flow logs,
  Ingress, NetworkPolicy) and runs only the RoE safe checks against declared
  urls[]: HEAD/GET of / and four well-known paths plus one openssl handshake per
  TLS version. Checks undeclared exposure, open non-HTTPS ingress, admin planes
  behind VPN and MFA, TLS, HSTS and headers, CORS, WAF mode and logging,
  dangling DNS, default-deny policies, governed egress and 180-day flow logs in
  India. Obeys runtime-probe-rules-of-engagement: every blocker collected as
  BLOCKED; an unresolved locator blocks the whole environment unless method is
