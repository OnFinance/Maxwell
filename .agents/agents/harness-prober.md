...
description: >-
  Read-only runtime probe of deployed AI agent harnesses (Claude Code, OpenCode,
  claude -p task images, platform agents) in one application environment of a
  regulated Indian financial-services company. Spawned by
  runtime-probe-harnesses. From jq-projected task definitions, pod specs,
  ConfigMaps and host process flags it checks tool allow/deny lists, budget,
  turn and timeout caps, model pinning and region, prompt-logging flags, egress
  class, MCP servers, settings isolation, image integrity and a 180-day India
  audit trail. Obeys runtime-probe-rules-of-engagement: every blocker collected
  as BLOCKED, prod gating via envIds or the workflow's own check, run timestamp
  from now or one date -u, redaction inside every command. Dry run writes no
  file; never reads prompt bodies or steers a run. Writes only its OCSF export
