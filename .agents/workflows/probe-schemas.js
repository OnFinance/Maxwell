// probe-schemas: static probe of data, API and event schema in every application repo checkout for one company.
...
// Shape: Scout (repos with checkouts, applicable instruments, run timestamp) -> per-repo pipeline: schema-auditor probe ->
...
export const meta = {
  name: 'probe-schemas',
  description: 'Static probe of database migrations, ORM models, OpenAPI/GraphQL and event schemas per app repo; unprotected PII/SPDI and weak contracts mapped to DPDP Rules 2025, SEBI CSCRF, RBI.',
  phases: [
    ...
    { title: 'Probe', detail: 'schema-auditor reviews each checkout, exports SARIF, returns candidate observations and findings' },
...
const WORKFLOW = 'probe-schemas';
const PROBE_AGENT = 'schema-auditor';
const BASE_TAGS = ["schema","data-protection","static-probe"];
const TARGET_NOUN = 'data, API and event schema';
const SUMMARY_NOUN = 'data and API schema gaps';
const MAPPING_RULE = '';
const requiredMappingGap = () => '';
const SCOUT_EXTRA = '';
const TARGET_RULE = 'For every applications/<app_id>/repos/<repo_id>.json decide whether it is a target. A repo is a target when its checkout contains schema-bearing files: SQL or ORM migrations (migrations/, db/migrate, alembic/, prisma/schema.prisma, *.sql, flyway/liquibase changelogs), API contracts (openapi*.yaml|json, swagger*, *.graphql, *.proto), event or message schemas (*.avsc, asyncapi*, JSON Schema under schemas/), or data-validation models (pydantic, zod, joi, class-validator). Skip repos with none of these and log the reason. For each target list the schema roots relative to the checkout and note the languages from the repo record.';
const WRITE_EXTRA = '';
const PROBE_CHECKS = [
  'Checklist:\n- Sensitive fields: columns or properties holding PAN, Aadhaar (UID/VID), passport, bank account, IFSC, UPI VPA, card PAN/CVV, date of birth, biometric, health or income data (SPDI under IT Rules 2011; personal data under DPDP Act 2023) stored without an encryption, tokenisation or masking annotation, or logged in plain text via audit tables.\n- Data minimisation and retention: tables collecting more personal data than the stated purpose, no retention/expiry column or policy for personal data, soft-delete only (no purge) for data DPDP requires erasing, consent, notice or purpose fields absent where DPDP Rules 2025 expect them (Rule 3 notice, Rule 6 reasonable security safeguards, Rule 8 erasure and retention periods).\n- Integrity: missing primary keys, foreign keys or unique constraints on ledger/transaction tables, monetary amounts stored as float/double instead of decimal/numeric, missing NOT NULL on regulatory identifiers, no audit columns (created_by, updated_at) on financial records, migrations that drop or truncate regulated tables without a backfill.\n- API contracts: OpenAPI operations without security schemes, additionalProperties left open on request bodies that reach the database, missing maxLength/pattern/enum on user-supplied strings, responses that echo full PAN/Aadhaar instead of masked values, no rate-limit or idempotency headers on payment endpoints, GraphQL introspection enabled for prod schemas, proto/Avro fields carrying secrets.\n- Localisation and residency hints: connection strings or schema comments pointing to foreign regions for payment data.',
  'Use Grep and Read over the schema roots; run the tools your agent definition permits when installed: spectral lint -f sarif for OpenAPI/AsyncAPI, sqlfluff lint or squawk for SQL migrations, buf lint for protobuf, semgrep --config p/secrets --sarif. Where no tool exists write the SARIF yourself (tool name "maxwell-schema-auditor"). Never connect to a live database.',
  'Cite dpdp-rules-2025 and dpdp-act-2023 first for personal-data gaps, sebi-cscrf-2024 (PR.DS.S1 data-at-rest encryption, ID.AM.S5 data inventories, PR.AA.S13 retention and disposal) for SEBI REs, rbi-cyber-tech-directions-2026 or rbi-digital-payment-security-2021 for payment data, pci-dss-4.0.1 (Req 3) where card data appears, irdai-info-cyber-security-2023 for insurers; then owasp-asvs-5.0 (V2 validation and business logic, V14 data protection) and nist-800-53-r5 (SC-28, SI-10).',
];
