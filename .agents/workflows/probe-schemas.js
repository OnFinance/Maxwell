...
const PROBE_CHECKS = [
  ...
  'Use Grep and Read over the schema roots; run scanners only through node .claude/scripts/toolchain/scan.mjs at their pinned versions (scanner-toolchain skill): spectral lint -f sarif for OpenAPI/AsyncAPI, sqlfluff lint or squawk for SQL migrations, buf lint for protobuf, gitleaks detect --no-git --redact for secrets. Where scan.mjs cannot run a tool (exit 3) write the SARIF yourself (tool name "maxwell-schema-auditor"). Never connect to a live database.',
