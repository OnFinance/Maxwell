// probe-cicd-env: static probe of CI/CD pipeline and build environment in every application repo checkout for one company.
...
// Shape: Scout (repos with checkouts, applicable instruments, run timestamp) -> per-repo pipeline: cicd-auditor probe ->
...
export const meta = {
  name: 'probe-cicd-env',
  description: 'Static probe of CI/CD definitions per app repo: action pinning, secrets, OIDC, SBOM presence (empty SBOM = missing), signing, scan gates, branch protection; mapped to SEBI CSCRF GV.SC and NIST SSDF.',
  phases: [
    ...
    { title: 'Probe', detail: 'cicd-auditor reviews each checkout, exports SARIF, returns candidate observations and findings' },
...
const WORKFLOW = 'probe-cicd-env';
const PROBE_AGENT = 'cicd-auditor';
const BASE_TAGS = ["cicd","supply-chain","static-probe"];
const TARGET_NOUN = 'CI/CD pipeline and build environment';
const SUMMARY_NOUN = 'CI/CD and supply-chain gaps';
const MAPPING_RULE = 'SBOM rule: an SBOM that is present but empty (zero bytes, {}, or a CycloneDX document with components: []) counts as a MISSING SBOM; a finding that treats such a file as satisfying sebi-cscrf-2024:GV.SC.S5 is wrong, and a missing-SBOM finding must cite GV.SC.S5 for SEBI REs, or the supply-chain or SBOM clause of the company\'s own applicable instrument (rbi-cyber-tech-directions-2026 or irdai-info-cyber-security-2023), and take the SLA for topic sbom.';
// SBOM findings must cite a supply-chain clause. Accepted: catalog controls Scout found that carry an SBOM requirement,
// SEBI CSCRF GV.SC.*, SSDF PS.3.*, or any clause of the RBI/IRDAI instruments whose catalogs may not exist yet.
const SBOM_INSTRUMENTS = ['rbi-cyber-tech-directions-2026', 'irdai-info-cyber-security-2023'];
const requiredMappingGap = (f) => {
  const tags = f.tags || [];
  const isSbom = tags.includes('sbom-missing') || tags.includes('sbom-empty');
  if (!isSbom) return '';
  const ok = (f.regulatoryRefs || []).some((r) => r && (mappingControlIds.has(r.instrument + ':' + r.controlId)
    || (r.instrument === 'sebi-cscrf-2024' && /^GV\.SC\./.test(r.controlId || ''))
    || (/^nist-ssdf-800-218a?$/.test(r.instrument || '') && /^PS\.3/.test(r.controlId || ''))
    || SBOM_INSTRUMENTS.includes(r.instrument)));
  return ok ? '' : 'SBOM finding cites no supply-chain clause (sebi-cscrf-2024 GV.SC, SSDF PS.3, an SBOM control of the catalogs, or rbi-cyber-tech-directions-2026 / irdai-info-cyber-security-2023)';
};
const SCOUT_EXTRA = 'Also return mappingControlIds: the instrument-qualified ids of every control in those catalogs whose title, prose or guidance requires a software bill of materials (SBOM) or component provenance, plus "<instrumentId>:<controlId>" for every hardRequirements entry with topic "sbom" (and a controlId) of an applicable instrument in instruments.json.';
const TARGET_RULE = 'For every applications/<app_id>/repos/<repo_id>.json decide whether it is a target. A repo is a target when its record has "ciSystem" other than "none", or when the checkout contains .github/workflows/*.yml, .gitlab-ci.yml, Jenkinsfile, azure-pipelines.yml, .circleci/config.yml, .buildkite/, cloudbuild.yaml, tekton/ or argo workflow manifests. Repos whose record says ciSystem "none" but whose checkout contains a pipeline file are still targets: note the discrepancy in the target\'s notes. For each target also list applications/<app_id>/images/*.json image records whose builtFrom.repoId is this repo and the sbomPath of each.';
const WRITE_EXTRA = 'For SBOM findings (tags sbom-missing or sbom-empty) choose slaBasis with topic "sbom" per soc-ledger section 6; when .claude/skills/regulatory-catalogs/references/sla-table.json has no sbom row for the instruments in scope, use defaults[severity] with slaBasis {instrument: the most specific cited instrument, days}.';
const PROBE_CHECKS = [
  'Checklist:\n- Pinning: third-party actions, orbs, includes or containers referenced by mutable tag or branch instead of a full commit SHA or digest; unpinned base images in Dockerfiles used by CI.\n- Secrets and identity: long-lived cloud keys in secrets instead of OIDC federation, secrets echoed or passed as command-line arguments, pull_request_target or workflow_run with checkout of untrusted heads, permissions: write-all or missing top-level permissions: read; self-hosted runners without ephemeral configuration; unmasked tokens in logs.\n- Supply chain (read .claude/skills/reference-architectures/SKILL.md, sections 4, 5 and 7 of references/investigation-saver-drhp-offline-copy.md): SBOM step absent; SBOM present but EMPTY - a file that is {} or has zero components, or a CycloneDX document whose components array is empty, MUST be reported as a missing SBOM; no cosign sign and attest of images; no SLSA provenance attestation; Trivy/Grype scan not blocking on CRITICAL/HIGH; dependency review or SCA absent; artifacts not checksummed.\n- Also check the workspace SBOMs: for each image record listed by Scout, read applications/<app_id>/images/<image_id>.cdx.json - if it is absent, is {} , or has components: [] treat it as missing and raise a finding against the image target.\n- Gates: no required status checks in the repo record branchProtection, missing IaC lint (checkov/cfn-lint) with documented skips, tests optional on main, deploy jobs that run kubectl apply/helm upgrade directly instead of GitOps (argo-cd/flux), missing environment protection rules or manual approval for prod, release pins not verified before promotion.\n- Environment hygiene: caches or artifacts containing secrets, artifact retention undefined, debug logging (ACTIONS_STEP_DEBUG) enabled, curl | bash installers, network egress unrestricted for build jobs.',
  'Prefer the tools your agent definition permits when installed: zizmor --format sarif for GitHub Actions, actionlint, hadolint for Dockerfiles, checkov -d <root> -o sarif, trivy config --format sarif, semgrep --config p/github-actions --sarif. For SBOM checks read the CycloneDX JSON with jq (jq \'.components | length\' <file>; a parse error, {} or 0 means empty). Where no tool exists write the SARIF yourself (tool name "maxwell-cicd-auditor").',
  'Cite the most specific Indian instrument first: sebi-cscrf-2024 for SEBI REs (GV.SC.S5 SBOM, PR.IP.S2 SDLC, PR.IP.S3 change control, PR.IP.S4 malicious-code scanning, PR.DS.S5 separation of non-production, PR.DS.S6 integrity verification, ID.RA.S1 and PR.IP.S12 vulnerability management, PR.AA.S3 least privilege for pipelines), rbi-cyber-tech-directions-2026 and rbi-it-outsourcing-md-2023 (App-I.6(a), App-I.6(f)) for banks/NBFCs, irdai-info-cyber-security-2023 for insurers, cert-in-directions-2022 (Dir-iv) where log retention of build systems is affected, dpdp-rules-2025 6(1)(b) where pipelines reach personal data; then nist-ssdf-800-218 (PO.3, PS.1, PS.3, PW.4, RV.1) and cis-controls-8.1 (16.x). SBOM findings take the SLA for topic sbom per soc-ledger section 6 (defaults[severity] when sla-table.json has no matching row).',
  'Present-but-empty SBOM rule: an SBOM file that exists but is empty ({}, zero bytes, or CycloneDX with an empty components array) is evidence of a MISSING SBOM, never of a satisfied control. Record the control observation as not-satisfied and raise the finding with tags "sbom-missing" and "sbom-empty"; an SBOM that is absent altogether gets tag "sbom-missing". Both target the image (targetType image, targetId <imageId>, fingerprint targetKey image:<appId>/<imageId>, path = the Dockerfile or pipeline file that should generate the SBOM) when an image record exists, otherwise the repo.',
];
...
const scout = await agent(
  [
    ...
    'Read company-profile/' + companyId + '/details.json: return entityTypes and frameworksInScope. Read .claude/skills/regulatory-catalogs/references/instruments.json and return applicableInstruments: the instrumentIds in frameworksInScope whose applicability covers the company (applicability.entityTypes empty or intersecting entityTypes; applicability.categories, when present, intersecting details.json regulatoryRegistrations[].category; applicability.jurisdictions empty or containing IN when the company is Indian).',
