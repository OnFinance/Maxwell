...
RESEARCH BUDGET: before you mark a 'public' question open, run at least 3 targeted WebSearch queries for it (the legal name or brand plus the topic; site:mca.gov.in master data and filings; the annual report or board's report PDF; AMFI or SEBI scheme and intermediary pages; NSE and BSE member directories and announcements; SEBI SCORES and the company's grievance page; the company's fair practices code, KFS, policies and 'about' pages) and fetch the best hits; list every query in queriesTried. 'internal' questions get one quick check of the company's website and the workspace, then status open with reason 'internal: ask the compliance officer'. Prefer sources a verifier can open again (web pages, investor presentations, policy PDFs, BSE and NSE filings, AMFI and SEBI pages); an annual report PDF over 10 MB cannot be re-fetched, so when you rely on one also cite a smaller source for the same fact where one exists and quote the page number in the evidence description. Answer each question from pages you fetch this session: the company's own website${skeleton.identifiers && skeleton.identifiers.cin ? ', MCA master data' : ''}, annual report and filings, AMFI or SEBI scheme lists, exchange member pages, regulator registers, and the workspace files applications/*/README.md and env/*.json (evidence type workspace-file). A question no source answers is status 'open' with a reason; a question the obligation does not reach for this company is 'not-applicable' with a reason. Never guess.
...
const BATCH_VERDICT = { type: 'object', required: ['verdicts'], properties: { sessionId: STR, verdicts: { type: 'array', items: { type: 'object', required: ['questionId', 'refuted', 'reason'], properties: { questionId: STR, refuted: { type: 'boolean' }, unverifiable: { type: 'boolean', description: 'true when the cited source could not be opened (size limit, blocked, offline) so nothing was checked; refuted must then be false' }, reason: STR, checkedUrl: STR } } } } };
// Verdict batches stay small: a whole questionnaire with long answers and evidence overran the refuter's prompt.
const VERDICT_BATCH = 6;
const answerRefutePrompt = (q, lens, subset) => {
  const researched = subset.map((x) => ({ questionId: x.questionId, question: x.question, answer: SHORT(x.answer, 700), evidence: (x.evidence || []).map((e) => ({ type: e.type, ref: e.ref })), yields: x.yields.map((y) => `${y.type}:${y.id}`) }));
  return `${common('refuter')}
Read .claude/skills/maxwell-conventions/SKILL.md and ${REFERENCE} section 2 (adversarial verification).
${GROUNDING}
Keyed batch: check each researched answer below about ${snapshot.legalName} (${companyId}). One verdict per questionId. These are descriptions of how the company operates, not drift claims, so the rule differs from drift: refuted=true ONLY when you opened a cited source and it does not support the answer, contradicts it, or names another entity. When you could not open a source (PDF over the fetch limit, blocked page, timeout) set unverifiable=true and refuted=false and say which ref failed; 'unproven' is not 'false'. A company's own investor presentation, policy PDF, annual report, BSE or NSE filing, AMFI or SEBI page, or workspace file is an acceptable source, and two corroborating sources are better than one, never worse.
Lens '${lens}': ${lens === 'source-authenticity' ? 'fetch each answer\'s evidence refs yourself (WebFetch for URLs, Read for workspace-file refs). Refute when a source you opened is not one of the acceptable kinds, or does not actually support the answer or a yield it names. A third-party aggregator (a broker site, a news article) supports an answer only for facts it quotes from the company or a regulator; mark those low-confidence in reason rather than refuting.' : `refute when a source you opened names a different legal entity than '${snapshot.legalName}' (CIN/PAN/registration numbers from ${PROFILE}): a parent, subsidiary, sister company, namesake or a peer used as an illustration. The entity's own group-level report that covers this entity is acceptable.`}
...
const [verified, answerVerdicts] = await parallel([
  ...
  () => pipeline(questionnaires.filter((q) => q.questions.some((x) => x.status === 'answered' && x.answeredBy === 'research')), async (q) => {
    const researched = q.questions.filter((x) => x.status === 'answered' && x.answeredBy === 'research');
    const chunks = [];
    for (let i = 0; i < researched.length; i += VERDICT_BATCH) chunks.push(researched.slice(i, i + VERDICT_BATCH));
    const votes = await parallel(chunks.flatMap((subset, c) => ANSWER_LENSES.map((lens) => () => agent(answerRefutePrompt(q, lens, subset), { label: `refute answers ${q.questionnaireId}${chunks.length > 1 ? ` #${c + 1}` : ''} [${lens}]`, phase: 'Verify', agentType: 'refuter', schema: BATCH_VERDICT, model: MODEL, effort: 'medium' }))));
    votes.forEach(noteSession);
    return { questionnaireId: q.questionnaireId, votes: votes.map((v, i) => ({ lens: ANSWER_LENSES[i % ANSWER_LENSES.length], verdicts: (v && v.verdicts) || null })) };
  }),
]);
...
for (const av of (answerVerdicts || []).filter(Boolean)) {
  ...
  for (const x of q.questions) {
    ...
    const mine = av.votes.flatMap((v) => (v.verdicts || []).filter((r) => r.questionId === x.questionId).map((r) => ({ ...r, lens: v.lens })));
    const against = mine.filter((r) => r.refuted && !r.unverifiable).map((r) => `${r.lens}: ${r.reason}`);
    if (!against.length) {
      // A source the refuter could not open leaves the answer standing at low confidence, marked in the record.
      if (mine.some((r) => r.unverifiable)) { x.confidence = 'low'; x.unverified = SHORT(mine.filter((r) => r.unverifiable).map((r) => `${r.lens}: ${r.reason}`).join(' | ')); }
      continue;
    }
    refutedAnswers += 1;
...
const contextDoc = {
  ...
  questionnaires: questionnaires.map((q) => ({ questionnaireId: q.questionnaireId, obligationId: q.obligationId, questions: q.questions.map((x) => ({ questionId: x.questionId, question: x.question, answerableFrom: x.answerableFrom, status: x.status, ...(x.status === 'answered' ? { answer: x.answer, answeredBy: x.answeredBy, confidence: x.confidence, evidence: (x.evidence || []).map((e) => ({ ...e, ...(e.description ? { description: SHORT(e.description) } : {}) })), answeredAt: now, ...(x.unverified ? { unverified: x.unverified } : {}) } : { reason: SHORT(x.reason || 'open') }), yields: x.yields })) })),
