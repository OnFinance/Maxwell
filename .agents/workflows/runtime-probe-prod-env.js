// runtime-probe-prod-env: read-only runtime probe of running container workloads (Kubernetes / Docker hosts) for one company.
// Tier-filtered and gated: only prod/dr environments; each is probed live only when args.envIds names it (without envIds every prod/dr environment is recorded as blocked PROD_GATING: inconclusive observation plus evidence request, RoE sections 1 and 8), probeAccess.readOnly must be true, changeFreeze and allowedWindows are honoured against args.now, rateLimitPerMinute is capped at 60 and passed to the prober, and the refuter adds a production-safety lens.
// args: { companyId (required), appIds?: string[], envIds?: string[] (needed for any live probe: every prod/dr environment explicitly, bare "prod-mumbai" or qualified "trading-api/prod-mumbai"; a bare id shared by several apps stays blocked), dryRun?: boolean,
...
export const meta = {
  name: 'runtime-probe-prod-env',
  description: 'Read-only runtime probe of running containers in prod and dr environments named explicitly in args.envIds; in-window, rate-limited (<=60/min), with a production-safety refute lens.',
  phases: [
...
const WORKFLOW = 'runtime-probe-prod-env';
const PROBE_AGENT = 'container-prober';
const TIER_GROUPS = { devtest: ['dev', 'test', 'devtest'], qa: ['qa', 'uat', 'staging'], prod: ['prod', 'dr'] };
const TIER_FILTER = ['prod']; // null = every tier group; otherwise the only group this workflow may touch
const REQUIRE_ENV_IDS = true;
