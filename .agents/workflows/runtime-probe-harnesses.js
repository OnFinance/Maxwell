// runtime-probe-harnesses: read-only runtime probe of deployed AI agent harnesses (Claude Code, OpenCode, custom claude -p
// or Bedrock harness images) for one company, environment by environment, using the harness-prober agent.
// Scout: applications whose repos have containsAgentCode (or that ship a harness image) x their env files. Refute: evidence
// + regulatory-mapping lenses; findings survive a strict majority, high/critical findings, risks and incidents need both;
// assessed observations face the evidence lens in one batch per environment; rules-of-engagement section 7 safety records
// (read-write credential finding, unredactable-data risk, probe-side-effect incident) are written without refutation.
...
// credentialKey, changeFreeze, allowedWindows, rateLimitPerMinute; every failed check becomes a blocker) -> pipeline per
// (app, env): prober -> refuter lenses -> Dedup barrier -> soc-ledger-keeper (append.mjs with soc-ledger reconciliation,
// then version.mjs) -> report-writer (summary.md).
...
export const meta = {
  name: 'runtime-probe-harnesses',
  description: 'Read-only live probe of deployed agent harnesses: tool allow-lists, budgets, turn caps, model pinning, egress class, MCP servers and audit trail, mapped to SEBI CSCRF, RBI 2026 and OWASP Agentic.',
  phases: [
    { title: 'Scout', detail: 'harness-prober in scout mode lists (app, env) targets whose repos have containsAgentCode or ship a harness image and fixes one run timestamp; the workflow records every failed prod-gating, readOnly, method, credential, freeze and window check as a blocker' },
    { title: 'Probe', detail: 'harness-prober resolves the credential locator and runs its HAR-01..HAR-11 read-only checks within the rate limit, or plans them in dry-run and blocked modes, returning candidate records keyed by ruleId and evidence requests' },
...
const WORKFLOW = 'runtime-probe-harnesses';
const PROBE_AGENT = 'harness-prober';
const CHECK_PREFIX = 'HAR';
...
const DOMAIN = 'agent-harness';
const ROE_SECTION = 'harnesses';
const BASE_TAGS = ['runtime-probe', 'agent-harness'];
const BASE_LENSES = ['evidence', 'regulatory-mapping'];
const SEVERE_LENSES = BASE_LENSES;
const PROBE_METHODS = ['kubeconfig', 'cloud-api', 'ssh'];
const RW_RULE = 'ROE-RW-CREDENTIAL'; // harness-prober section 8 names the RoE rule id verbatim
const RW_CHECK_ID = 'HAR-RW';
const ALLOWED_TIERS = null; // cross-cutting probe: inherits the tier rule of the environment it touches (prod gating applies)
const LOCATION_EXAMPLES = 'ecs/task-definition/<family>, <namespace>/deployment/<name>, host/<alias>/<unit>, image/<imageId>, mcp/<server-name>';
const RULE_EXAMPLES = '"harness-tool-allowlist-missing", "harness-egress-class-undeclared", "harness-audit-trail-gap"';
const EXTRA_READS = ', .claude/skills/reference-architectures/references/investigation-saver-drhp-offline-copy.md sections 3 and 7 item 2 (harness catalogue, egress classes, budget grants, governance tables; the model of good, graded on outcomes)';
const regmapHint = () => 'For agent-harness gaps expect catalog ids per harness-prober section 4, most specific first: sebi-cscrf-2024 PR.AA.S15 (access control), PR.AA.S3 (least privilege) and GV.RR.S2 (roles) for HAR-01, HAR-02, HAR-03 and HAR-10; GV.SC.S4 (third-party monitoring) and, for RBI-regulated entities, rbi-it-outsourcing-md-2023 8 (inventory of outsourced services) for the model provider in HAR-04; cert-in-directions-2022 Dir-iv with sebi-cscrf-2024 PR.AA.S8, PR.AA.S9 and DE.CM.S2 for HAR-05 and HAR-08; PR.AA.S2 (network segmentation), ID.AM.S3 (no shadow IT) and PR.AA.S17 (API security) for HAR-06 and HAR-07; GV.SC.S5 (SBOM), PR.DS.S6 (integrity verification) and PR.IP.S15 for HAR-09 and HAR-11; dpdp-rules-2025 6(1)(b), 6(1)(c), 6(1)(e) and 15 with readiness wording (rules commence 13 May 2027) when prompts can carry personal data. Instruments without a catalog file (rbi-cyber-tech-directions-2026, rbi-it-governance-md-2023) are omitted, never cited with a guessed id. Then global mappings: nist-csf-2.0 PR.PS-01 / PR.IR-01, owasp-agentic-top10-2026, owasp-llm-top10-2025 LLM02, csa-mcp-security-2025, nist-ai-600-1, nist-ssdf-800-218 PS.3.2. PR.PS and PR.IR are NIST CSF 2.0 ids: under sebi-cscrf-2024 they are mapping errors.';
const defaultRateFor = () => 30;
const targetContext = (t) => ((t.agentRepoIds || []).length ? 'Agent-code repos: ' + t.agentRepoIds.join(', ') + '. ' : '') + ((t.harnessImageIds || []).length ? 'Harness image records: ' + t.harnessImageIds.join(', ') + '.' : '');
const lensText = (t, c, lens) => 'Lens ' + lens + ': apply the refuter checks for this lens from your agent definition.';
const CHECKS = (t) => 'HAR-01 tool allow/deny (ruleId harness-tool-allowlist-missing): an explicit allow-list per image, WebSearch (native web search) denied everywhere, browser and document variants forbid Agent, Bash, Computer, Cron*, Exec, Execute, Process, Shell, Spawn, Task*, Terminal and Workflow, no bypassPermissions or --dangerously-skip-permissions in qa/prod; '
  + 'HAR-02 budget ceiling (harness-budget-unbounded): a per-run USD or token ceiling with bounded increases (grant history unknown without evidence); '
  + 'HAR-03 turn caps and timeouts (harness-turn-cap-missing): max turns, an execution timeout on the task definition or activeDeadlineSeconds, bounded concurrency; '
  + 'HAR-04 model pinning and region (harness-model-not-pinned): a dated or explicitly versioned model id per image, inference region inside residency ' + ((t.residency || []).join('/') || '(see env record)') + ' or a documented cross-border basis, approved-list membership unknown until the list is supplied by evidence request (sdlc/policy.json has no approved-model list); '
  + 'HAR-05 prompt and tool logging (harness-content-logging-enabled): prompt and tool-content logging off by default in prod, otherwise only to the India-resident SIEM with >= 180 days retention and a documented DPDP basis; '
  + 'HAR-06 egress class (harness-egress-class-undeclared): every image declares offline, loopback-only or governed, the network configuration matches it, and fetches leave only through the governed gateway or one approved HTTPS MCP server; '
  + 'HAR-07 MCP servers (harness-mcp-unreviewed-server): reviewed servers only, pinned, stdio or HTTPS, credentials by {env:...} reference, --strict-mcp-config set, no write-capable server on a read-only harness; '
  + 'HAR-08 audit trail (harness-audit-trail-gap): every run leaves session events and a harness snapshot pinning the task-definition revision and image digest, logs retained >= 180 days in India (>= 365 days when prompts may carry pii/spdi), alerts reach on-call; '
  + 'HAR-09 image integrity (harness-image-unpinned-or-unattested): harness images run by registered digest, signed and attested with a non-empty SBOM, rebuilt within 90 days; '
  + 'HAR-10 brakes and human apply (harness-governor-applies-changes): governor-style agents propose and humans apply, *_paused brake flags exist and are readable; '
  + 'HAR-11 settings isolation (harness-host-settings-leak): --setting-sources "", --disable-slash-commands, --no-chrome, non-root numeric UID with a read-only root filesystem, tmpfs for workspace, home and tmp, an init process, and a single-use host or isolation lease where the catalogue expects one. '
  + 'Rules-of-engagement section 9 "harnesses" items map onto these ids only: egress class -> HAR-06; non-root, read-only root FS, tmpfs and init -> HAR-11; native web search -> HAR-01; isolation lease -> HAR-11; harness snapshot pinning -> HAR-08. The process-guard CI self-test is outside harness-prober\'s commands: record it as an evidence request under HAR-11 (runtime-probe-sandboxes SBX-07 evidences it on sandbox hosts). Use no other check ids.';
const EXTRA_PROHIBITIONS = () => 'never start, stop, steer or message an agent run; never change a governance flag, brake or budget grant; never run claude, opencode or any model call; never read prompt, transcript or tool-result bodies, so no kubectl logs and no aws logs get-log-events, filter-log-events or start-query on any tier; psql and every database client are forbidden by harness-prober\'s own allow-list (stricter than rules-of-engagement section 2), so budget-grant, lease and audit-table figures become evidence requests.';
const EXTRA_PROMPT = () => '';
const SCOUT_EXTRA_PROPS = {};
const SCOUT_TARGET_PROPS = { agentRepoIds: { type: 'array', items: { type: 'string' } }, harnessImageIds: { type: 'array', items: { type: 'string' } } };
const scoutScopeLines = [
  'An application is in scope when at least one applications/<app_id>/repos/<repo_id>.json has "containsAgentCode": true, or when an applications/<app_id>/images/<image_id>.json describes an agent harness image (name, labels or description mentioning claude, opencode, agent, harness or mcp). Every other app gets a skipped entry {appId, reason: "no repo with containsAgentCode and no harness image"}. For in-scope apps read every applications/<app_id>/env/<env_id>.json and add agentRepoIds and harnessImageIds to each target.',
];
const afterScout = () => {};
const extraBlockers = () => [];
const afterTargets = () => {};
